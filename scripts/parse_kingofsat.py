...
def detect_quality(row: Tag, name: str, kind: str) -> str:
    if kind == "RADIO":
        return "RADIO"

    markers = [name]
    markers.extend(clean(v) for v in row.get("class", []))
    ...
    text = " ".join(markers).upper().replace("ULTRA HD", "UHD")

    if re.search(r"(?:^|[^A-Z0-9])(?:UHD|4K|2160P)(?:[^A-Z0-9]|$)", text):
        return "UHD"
    if re.search(r"(?:^|[^A-Z0-9])(?:3DTV|3D)(?:[^A-Z0-9]|$)", text):
        return "3D"
    if re.search(r"(?:^|[^A-Z0-9])(?:HDTV|FULL HD|1080P|720P|HD)(?:[^A-Z0-9]|$)", text):
        return "HD"
    if re.search(r"(?:^|[^A-Z0-9])(?:SDTV|576I|480I|SD)(?:[^A-Z0-9]|$)", text):
        return "SD"
    return "UNKNOWN"
...
def parse_channel_row(row: Tag, tp: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        "name": name,
        "type": kind,
        "quality": detect_quality(row, name, kind),
